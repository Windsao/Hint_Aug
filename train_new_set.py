...
from engine_new import train_one_epoch, evaluate
...
def get_args_parser():
    ...
    parser.add_argument('--start_adv', default=-1, type=int)
    parser.add_argument('--new_set', default='', type=str)
    return parser
...
def main(args):
    ...
    # if args.new_set != '':
    #     base_file = os.path.join('/home/sw99/CDFSL-ATA/filelists', args.new_set, 'novel.json')
    #     val_file = os.path.join('/home/sw99/CDFSL-ATA/filelists', args.new_set, 'novel.json')
    #     image_size = 224
    #     n_query = max(1, int(16*5/5))
    #     base_datamgr = SetDataManager(image_size, n_query=n_query, n_way=5, n_support=args.few_shot_shot)
    #     data_loader_train = base_datamgr.get_data_loader(base_file, aug=True)
    #     val_datamgr = SetDataManager(image_size, n_query=n_query, n_way=5, n_support=args.few_shot_shot)
    #     data_loader_val = val_datamgr.get_data_loader(val_file, aug=False)
    #     if args.new_set == 'EuroSAT':
    #         args.nb_classes = 10
    #     else:
    #         args.nb_classes = 38
    # else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if epoch > 1:
        if epoch > 30 and epoch % args.val_interval == 0 or epoch == args.epochs-1:
