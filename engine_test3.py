...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    teach_loss: torch.nn.Module = None, choices=None, mode='super', retrain_config=None,is_visual_prompt_tuning=False,is_adapter=False,is_LoRA=False,is_prefix=False, classes=100, gen_model=None, cf_matrix=None, eps=0.01):
    model.train()
    criterion.train()
    ...
    # atk.set_mode_targeted_least_likely(classes - 1)
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # m = nn.Softmax()
        # rand_labels = torch.randn_like(targets)
        # max_targets = m(rand_labels).max(1)[1]
        # print(rand_labels, max_targets)
        # exit()
        ...
        # label_left = F.one_hot(tar_labels, num_classes=classes)
        label_left = targets
        '''
        ...
        if cf_matrix is not None:
            ...
            for count, each_label in enumerate(new_labels):
                ...
                if cf_matrix[which_class].sum() != 0:
                    dist = torch.from_numpy(cf_matrix[which_class])
                    one_hot_class = torch.multinomial(dist, 1)
                    new_labels[count] = F.one_hot(one_hot_class, num_classes=classes)
