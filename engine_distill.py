...
def train_one_epoch_delta(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if args.patch_fool:
            mask = get_attn_mask(model, samples, targets, args)
        else:
            mask = torch.zeros_like(samples).cuda()
            mask[..., 0:16, 0:16] = 1
        patch = delta * mask
        ...
        if amp:
            ...
            if args.patch_fool:
                outputs, attn = model(samples + patch)
            else:
                outputs = model(samples + patch)
            ...
            if teacher_model:
                with torch.no_grad():
                    if args.patch_fool:
                        ...
                        teach_output = teacher_model(samples + patch)
                _, teacher_label = teach_output.topk(1, 1, True, True)
                loss = 1 / 2 * criterion(outputs, targets) + 1 / 2 * teach_loss(outputs, teacher_label.squeeze())
            else:
                loss = criterion(outputs, targets)
...
def evaluate(data_loader, model, device, amp=True, choices=None, mode='super', retrain_config=None,is_visual_prompt_tuning=False,is_adapter=False,is_LoRA=False,is_prefix=False, args=None, delta=None):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        if delta is not None:
            if args.patch_fool:
                mask = get_attn_mask(model, images, target, args)
            else:
                mask = torch.zeros_like(images).cuda()
                mask[..., 0:16, 0:16] = 1
            patch = delta * mask
            if args.patch_fool:
                n_output, n_attn = model(images + patch)
            else:
                n_output = model(images + delta)
            
            n_acc1, n_acc5 = accuracy(n_output, target, topk=(1, 5))
            metric_logger.update(noise_acc1=n_acc1.item())
            metric_logger.update(noise_acc5=n_acc5.item())
            # metric_logger.meters['noise_acc1'].update(n_acc1.item(), n=batch_size)
            # metric_logger.meters['noise_acc5'].update(n_acc5.item(), n=batch_size)
