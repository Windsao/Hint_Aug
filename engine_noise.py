...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    teach_loss: torch.nn.Module = None, choices=None, mode='super', retrain_config=None,is_visual_prompt_tuning=False,is_adapter=False,is_LoRA=False,is_prefix=False, args=None):
    model.train()
    criterion.train()
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if amp:
            ...
            if teacher_model:
                with torch.no_grad():
                    if args.patch_fool:
                        eps = torch.tensor([8/255., 8/255., 8/255.]).view(3, 1, 1).cuda()
                        delta = torch.randn_like(samples)
                        delta.data = clamp(delta, -eps, eps)
                        delta.requires_grad = True
                        mask = torch.zeros_like(samples)
                        mask[..., 0:16, 0:16] = 1
                        delta = delta * mask
                        teach_output, t_attn = teacher_model(samples + delta)
                    else:
                        teach_output = teacher_model(samples)
                _, teacher_label = teach_output.topk(1, 1, True, True)
                loss = 1 / 2 * criterion(outputs, targets) + 1 / 2 * teach_loss(outputs, teacher_label.squeeze())
...
@torch.no_grad()
def evaluate(data_loader, model, device, amp=True, choices=None, mode='super', retrain_config=None,is_visual_prompt_tuning=False,is_adapter=False,is_LoRA=False,is_prefix=False, args=None):
